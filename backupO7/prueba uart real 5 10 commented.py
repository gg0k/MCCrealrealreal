...
class Scope(QWidget):
    ...
    def __init__(self,filename,port=None):
        ...
        self.alpha_intervals = [(89, 98.6), (98.6, 107), (107, 116.2), (116.2, 125.2), (125.2, 134), (134, 143.2),
                                (143.2, 152), (152, 170.8)]
        self.alpha_list=[]
        self.alpha_accumulated_data = {interval: [] for interval in self.alpha_intervals}
        self.average_alpha=0
        #print(port)
    ...
    def adjust_sensitivity(self,average_alpha, current_time):
        # Ajuste de sensibilidad SOLO en intervalos específicos
        if 98.6 <= current_time < 107:
            # En el intervalo de 98.6-107, ajustamos la sensibilidad para mantener un promedio alto
            if average_alpha < 30:
                sensitivity_factor = 10  # Bajamos un poco la sensibilidad si el promedio es bajo
            elif average_alpha > 40:
                sensitivity_factor = 20  # Subimos un poco la sensibilidad si el promedio es alto
            else:
                sensitivity_factor = 15  # Mantenemos sensibilidad media
        elif 107 <= current_time < 116.2:
            # En el intervalo de 107-116.2, ajustamos la sensibilidad para mantener un promedio bajo
            if average_alpha < 30:
                sensitivity_factor = 25  # Subimos un poco la sensibilidad si el promedio es bajo
            elif average_alpha > 40:
                sensitivity_factor = 15  # Bajamos un poco la sensibilidad si el promedio es alto
            else:
                sensitivity_factor = 20  # Sensibilidad media
        elif 116.2 <= current_time < 125.2:
            # En el intervalo de 116.2-125.2, intentamos estabilizar el promedio
            if average_alpha < 30:
                sensitivity_factor = 15  # Bajamos un poco la sensibilidad si el promedio es bajo
            elif average_alpha > 40:
                sensitivity_factor = 25  # Subimos un poco la sensibilidad si el promedio es alto
            else:
                sensitivity_factor = 20  # Sensibilidad media
        else:
            # Fuera de los intervalos, mantenemos el valor por defecto (sin ajustes)
