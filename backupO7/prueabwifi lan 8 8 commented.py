...
class Scope(QWidget):
    ...
    def __init__(self):
        ...
        self.xRange_slider = Slider(1, 2000, 2000)
        ...
        self.notch_slider = Slider(30, 55, 50)
        ...
        self.esp32_ip = '192.168.1.109'
        self.port = 80
        self.s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.s.connect((self.esp32_ip, self.port))
        print(f"Conectado al ESP32 en {self.esp32_ip}:{self.port}")
    ...
    def serial_read(self):
        ...
        while True:
            line=self.s.recv(1024).decode('utf-8').strip()
            #print(line)
            top_indices = np.argsort(self.fft_graph_fft_mag)[-100:][::-1]
            top_frequencies = self.fft_freq[top_indices]
            for freq in top_frequencies:
