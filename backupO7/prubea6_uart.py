...
class Scope(QWidget):
    def __init__(self, port):
        ...
        self.lowpass_slider = Slider(1, 500, 100)  # Lowpass slider, initial value should not be a fraction
        self.horizontalLayout.addWidget(self.lowpass_slider)
        self.highpass_slider = Slider(0.00001, 0.0001, 0.00001)  # Highpass slider, initial value should not be a fraction
        self.horizontalLayout.addWidget(self.highpass_slider)
        self.notch_slider = Slider(160, 170, 160)  # Notch filter slider
        ...
        # Serial port for data collection
        self.host = '192.168.223.74'  # Reemplaza con la dirección IP del ESP32
        self.port = 80
        ...
        self.plot_timer.start(1)  # 2 ms timer
    ...
    def serial_read(self):
        ...
        while True:
            line = self.s.recv(1024).decode().strip()
            if line:
                try:
                    value = line.split('\n')
                    for dat in value:
                        if dat:
                            parts = dat.split(':')
                            if len(parts) == 2:
                                lectura = float(parts[1])

                                if self.ptr < self.buffer_size:
                                    self.data[self.ptr] = lectura
                                    self.ptr += 1
                                else:
