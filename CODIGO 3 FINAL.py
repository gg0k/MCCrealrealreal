...
class MCC(QMainWindow):
    ...
    def update_plot(self):
        if self.graphState:
            ...
            #     emg = np.mean(data)
            #
            #     # Si estamos esperando estabilización y el valor está cerca de 0, restablecemos
            #     if self.wait_for_stabilization and self.emg_threshold_lower < emg < self.emg_threshold_upper:
            #         self.wait_for_stabilization = False
            #         self.last_emg_state = 0
            #         print(0)
            #
            #     # Lógica de detección con histéresis
            #     elif not self.wait_for_stabilization:
            #         if emg <= self.emg_threshold_lower and self.last_emg_state != -1:  # Señal negativa
            #             print(-1)
            #             self.last_emg_state = -1
            #             self.wait_for_stabilization = True  # Esperar estabilización antes de otro pulso
            #
            #         elif emg >= self.emg_threshold_upper and self.last_emg_state != 1:  # Señal positiva
            #             print(1)
            #             self.last_emg_state = 1
            #             self.wait_for_stabilization = True  # Esperar estabilización antes de otro pulso
            # elif self.signalSelector1.currentText()=="EOG":
            #     emg = np.mean(data)
            #     if emg <= 1.58:
            #         print(-1)
            #     elif emg >= 1.58 and emg <= 1.72:
            #         print(0)
            #     elif emg >= 1.72:
            #         print(1)
